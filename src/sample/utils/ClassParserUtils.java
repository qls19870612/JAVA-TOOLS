...
public class ClassParserUtils {
    private static Pattern classPattern = Pattern.compile("public\\s+(?:\\w+\\s)*class\\s+(\\w+)");
    public static Pattern packageNamePattern = Pattern.compile("package\\s+(([\\s\\S])+?);");
    public static Pattern enumPattern = Pattern.compile("public\\s+enum\\s+(\\w+)");
