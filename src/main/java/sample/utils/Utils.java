...
public class Utils extends CommonUtils {
    ...
    public static byte[] snappyCompress(byte[] data) {
        ...
        int compressedSize = Snappy.compress(data, 0, data.length, compressed, 0);
    ...
    public static void shuffle(int[] arr) {
        for (int index = arr.length; index > 1; index--) {
            int i = index - 1;
            int j = RandomNumber.getRate(index);
    ...
    public static <T> void shuffle(List<T> arr) {
        for (int index = arr.size(); index > 1; index--) {
            int i = index - 1;
            int j = RandomNumber.getRate(index);
    ...
    public static <T> T[] collectSpecificObjs(Injector injector, Class<T> cls) {
        List<T> list = GuiceFilter.filter(injector, cls);
